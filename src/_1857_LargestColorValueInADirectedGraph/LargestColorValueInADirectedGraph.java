...
public class LargestColorValueInADirectedGraph {
    ...
    private static int largestPathValueV1(String colors, int[][] edges) {
        int n = colors.length();
        List<List<Integer>> adj = new ArrayList<>();
        for(int i = 0; i < n; i++) {
            adj.add(new ArrayList<>());
    ...
    private static int dfsV1(int node, String colors, List<List<Integer>> adj, int[][] count
            , int[] vis) {
        if(vis[node] == 1) {
            return INF;
        }
        ...
        vis[node] = 1;
        for(int nxt : adj.get(node)) {
            int res = dfsV1(nxt, colors, adj, count, vis);
            if(res == INF) {
                return INF;
            }
